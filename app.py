...
if trip_file and fuel_file:
    try:
        ...
        meta_trip = xl_trip.parse(xl_trip.sheet_names[0], header=None, nrows=20)

        # Find "Registration" row and extract value
        reg_row_idx = meta_trip[meta_trip.iloc[:, 0].astype(str).str.contains("Registration", na=False)].index
        registration = None
        if len(reg_row_idx) > 0:
            registration = str(meta_trip.iloc[reg_row_idx[0], 1]).strip()

        # Find start of data where "Driver" header exists
        raw_trip = xl_trip.parse(xl_trip.sheet_names[0], header=None)
        header_idx = raw_trip[raw_trip.iloc[:, 0] == "Driver"].index[0]

        # Read data table
        df_trip = xl_trip.parse(xl_trip.sheet_names[0], skiprows=header_idx)
